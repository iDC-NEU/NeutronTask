...
class GCN_no_Decouple_pipeline_impl {
public:
    int iterations;
    ...
    std::mutex P_mutex;
    std::mutex T_forward_mutex_1;
    std::mutex P_mutex_1;
    std::mutex T_backward_mutex;
    std::queue<int> T_forward_queue;
    std::queue<int> T_forward_queue_1;
    std::queue<int> P_queue;
    std::queue<int> P_queue_1;
    std::queue<int> T_backward_queue;
    
    double no_P_time = 0.0;
    
    GCN_no_Decouple_pipeline_impl(Graph<Empty> *graph_, int iterations_,
    ...
    void init_nn(){
        ...
        for (int i = 0; i < graph->gnnctx->layer_size.size() - 1; i++) {
            ...
            P1.push_back(new Parameter(graph->gnnctx->layer_size[i],
                                        graph->gnnctx->layer_size[i + 1], alpha, beta1,
                                        beta2, epsilon, weight_decay));
        }
        ...
        for (int i = 0; i < P.size(); i++) {
            ...
            P1[i]->init_parameter();
            P1[i]->set_decay(decay_rate, decay_epoch);
        }
    ...
    void init_decoupled_version(){
        ...
        pipeline_partitioner->alloc_GPU_handle_chunk();
            ...
            { 
                ...
                P[j]->to(GPU);
                P[j]->Adam_to_GPU();
                torch::Device GPU2(torch::kCUDA, 2);
                P1[j]->to(GPU2);
                P1[j]->Adam_to_GPU();
            }
    ...
    NtsVar vertexForward(NtsVar &x) {
        ...
        int layer = graph->rtminfo->curr_layer;
            ...
            if(layer == 0)
                ...
                y = torch::relu(P1[layer]->forward(x)).set_requires_grad(true);
    ...
    void decoupled_P_caching(int pip_id)
    {
        ...
        for(int layer = 0; layer < K; layer++)
        {
            ...
            graph_chunk->SPMM_csc_caching(layer);
            graph_chunk->load_embedding_to_next_one(layer);
        ...
        graph_chunk->X_PB[0] = graph_chunk->X_P[K];
        ...
        for(int layer = 0; layer < K; layer++)
        {
            graph_chunk->SPMM_csr_caching(layer);
            
            graph_chunk->load_grad_to_next_one(layer);
        }
        graph_chunk->cs->CUDA_DEVICE_SYNCHRONIZE();
        PB_t += get_time();
        gB_time[pip_id].push_back(PB_t);
    ...
    void no_decoupled_T(int pip_id, int layer)
    {
        GraphChunk *graph_chunk = pipeline_partitioner->chunks[pip_id];
        ...
        NN_t -= get_time();
            ...
            graph->rtminfo->curr_layer = layer;
            NtsVar a = vertexForward(graph_chunk->X_T[layer]);
        ...
        if(layer == 0)
        {
            graph_chunk->gpu_id = 1;
        }
        else
        {
            graph_chunk->gpu_id = 3;
        }
        graph_chunk->load_graph_to_P_gpu_nodecoupled();
        graph_chunk->send_embedding_nodecoupled(a, graph_chunk->X_P[layer]);
    ...
    void no_decoupled_P(int pip_id, int layer)
    {
        GraphChunk *graph_chunk = pipeline_partitioner->chunks[pip_id];
        double P_t = 0.0;
        P_t -= get_time();
            ...
            NtsVar a;
            double spmm_aggregate_time = 0.0;
            spmm_aggregate_time-= get_time();
            graph_chunk->SPMM_csc_nodecoupled(layer, a); 
            spmm_aggregate_time+= get_time();
            std::cout <<"pip_id:"<<pip_id<< "  spmm_aggregate_time: " << spmm_aggregate_time << std::endl;
        ...
        if(layer == 0)
        {
            graph_chunk->gpu_id = 2;
            graph_chunk->send_embedding_nodecoupled(a, graph_chunk->X_T[layer+1]);
        }
        else
        {
            graph_chunk->gpu_id = 3;
    ...
    void run_decoupled_version(){
        ...
        for (int i_i = 0; i_i < iterations; i_i++) 
        {
            ...
            std::function pipeline_T_1 = [&]
                {
                    for(int i = 0; i < num_pipelines; i++)
                    {
                        ...
                        no_decoupled_T(pipeline_id, 1);

                        T_forward_mutex_1.lock();
                        T_forward_queue_1.push(pipeline_id);
                        T_forward_mutex_1.unlock();
                    }
                };

            std::function pipeline_P_1 = [&]
                {
                    for(int step = 0; step < num_pipelines; step++)
                    {
                        while(1)
                        {
                            T_forward_mutex_1.lock();
                            bool condition = !T_forward_queue_1.empty();
                            T_forward_mutex_1.unlock();
                            if(condition)
                            {
                                break;
                        ...
                        T_forward_mutex_1.lock();
                        int pipeline_id = T_forward_queue_1.front();
                        T_forward_queue_1.pop();
                        T_forward_mutex_1.unlock();

                        no_decoupled_P(pipeline_id, 1);

                        P_mutex_1.lock();
                        P_queue_1.push(pipeline_id);
                        P_mutex_1.unlock();
            ...
            std::thread T_forward_thread_0(pipeline_T_0);
            std::thread P_thread_0(pipeline_P_0);
            std::thread T_forward_thread_1(pipeline_T_1);
            std::thread P_thread_1(pipeline_P_1);
            std::thread T_backward_thread(pipeline_T_B);

            T_forward_thread_0.join();
            P_thread_0.join();
            T_forward_thread_1.join();
            P_thread_1.join();
