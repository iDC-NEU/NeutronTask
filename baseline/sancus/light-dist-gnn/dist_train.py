...
def train(g, env, args):
    ...
    with open(args.savepath, 'a') as f:
        for epoch in range(args.epoch):
            ...
            # if epoch%10==0 or epoch==args.epoch-1: delete by lusz
                # 收集所有节点的输出，并拼接在一起
                ...
                if g.labels.dim()>1:
                    ...
                    # add by lusz
                    if env.rank == 0:
                        # acc_message = "{:.6f}".format(acc(g.val_mask))
                        acc_message = "{:.6f}".format(acc(g.test_mask))
                        print(acc_message)
                        f.write(acc_message+' ')
...
def main(env, args):
    ...
    print(f"Rank: {env.rank}, local vtx: {g.local_num_nodes}, local edge: {g.local_num_edges}")
